sap.ui.define([
	...
	return BaseObject.extend("com.company.wishlist.ui.model.GroupSortState", {
		 ...
		 * @alias com.company.wishlist.ui.model.GroupSortState
		 */
		...
		/**
		 * Sorts by productName, or by productPrice
		 *
		...
		/**
		 * Groups by productPrice, or resets the grouping for the key "None"
		 *
		 * @param {string} sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "productPrice") {
				// Grouping means sorting so we set the select to the same Entity used for grouping
				this._oViewModel.setProperty("/sortBy", "productPrice");

				aSorters.push(
					new Sorter("productPrice", false,
